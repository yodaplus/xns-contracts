...
async function setTLDsOnRoot(owner, root, registrar, tlds) {
  ...
  for (const tld of tlds) {
    ...
    transactions.push(
      await ethers
        .getContractAt("DNSRegistrar", registrar.address)
        .setSubnodeOwner(labelhash, registrar.address)
    );
  }
  return transactions;
}
...
async function setTLDsOnRegistry(owner, registry, registrar, tlds) {
  ...
  for (const tld of tlds) {
    ...
    transactions.push(
      await ethers
        .getContractAt("ENSRegistry", registrar.address)
        .setSubnodeOwner(ZERO_HASH, labelhash, registrar.address)
    );
  }
  return transactions;
}
